<#
    ...
    .EXAMPLE
        .\Run-E2ETest.ps1 -E2ETestFolder "C:\Data\e2etests" -ReleaseLabel "Release-ARM-1" -ReleaseArtifactImageBuildNumber "20190101.1" -Architecture "x64" -TestName "TempSensor" -ContainerRegistryUsername "EdgeBuilds" -ContainerRegistryPassword "xxxx" -IoTHubConnectionString "xxxx" -EventHubConnectionString "xxxx" -ProxyUri "http://proxyserver:3128"
    ...
    Function RunDirectMethodAmqpTest
    {
        ...
        $deviceId = "e2e-${ReleaseLabel}-Windows-DMAmqp-$(Get-Date -UFormat %s)"
    ...
    Function RunDirectMethodMqttTest
    {
        ...
        $testStartAt = Get-Date
        $deviceId = "e2e-${ReleaseLabel}-Windows-DMMqtt-$(Get-Date -UFormat %s)"
        PrintHighlightedMessage "Run quickstart test with -d ""$deviceId"" and deployment file $DeploymentWorkingFilePath started at $testStartAt"
    ...
    Function RunQuickstartCertsTest
    {
        ...
        $testStartAt = Get-Date
        $deviceId = "e2e-${ReleaseLabel}-Windows-QuickstartCerts-$(Get-Date -UFormat %s)"
        PrintHighlightedMessage "Run quickstart test with -d ""$deviceId"" and deployment file $DeploymentWorkingFilePath started at $testStartAt"
    ...
    Function RunTempFilterTest
    {
        ...
        $testStartAt = Get-Date
        $deviceId = "e2e-${ReleaseLabel}-Windows-tempFilter-$(Get-Date -UFormat %s)"
        PrintHighlightedMessage "Run quickstart test with -d ""$deviceId"" and deployment file $DeploymentWorkingFilePath started at $testStartAt"
    ...
    Function RunTempFilterFunctionsTest
    {
        ...
        $testStartAt = Get-Date
        $deviceId = "e2e-${ReleaseLabel}-Windows-tempFilterFunc-$(Get-Date -UFormat %s)"
        PrintHighlightedMessage "Run quickstart test with -d ""$deviceId"" and deployment file $DeploymentWorkingFilePath started at $testStartAt"
    ...
    Function RunTempSensorTest
    {
        ...
        $testStartAt = Get-Date
        $deviceId = "e2e-${ReleaseLabel}-Windows-tempSensor-$(Get-Date -UFormat %s)"
    ...
    Function RunTransparentGatewayTest
    {
        ...
        $testStartAt = Get-Date
        $deviceId = "e2e-${ReleaseLabel}-Windows-TransGW-$(Get-Date -UFormat %s)"
        PrintHighlightedMessage "Run quickstart test with -d ""$deviceId"" started at $testStartAt."
